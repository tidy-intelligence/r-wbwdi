---
title: "Using `wbwdi` with `wbids`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Using `wbwdi` with `wbids`}
...
```{r, include=FALSE}
library(httptest)
start_vignette("using-wbwdi-with-wbids")
```
...
The `wbwdi` packages focuses on providing access to World Bank Development Indicators (WDI), which focus on observations per geography and year. The `wbids` package, on the other hand, provides access to International Debt Statistics (IDS), which are available by geography, counterpart, and year. However, both packages have a shared design philosophy in order to work seamlessly together. 
...
```{r, warnings=FALSE, fig.dim=c(7, 4)}
...
government_debt <- wdi_get(
  "ZMB", c("NY.GDP.MKTP.CD", "GC.DOD.TOTL.GD.ZS"),
  start_year, end_year, format = "wide"
)
...
government_debt <- government_debt |>
  ...
  select(geography_id, year, debt, type)

# Fetch total external debt in USD
external_debt <- ids_get(
  "ZMB", "DT.DOD.DPPG.CD", "all",
  start_year, end_year
)
...
debt_levels <- external_debt |>
  ...
  select(geography_id, year, debt = value) |>
...
debt_levels |>
  ...
  geom_line() +
  labs(x = NULL, y = "Debt Stock (in billion USD)", color = NULL,
...
debt_breakdown <- external_debt |>
  ...
  left_join(
    ids_list_counterparts(), join_by(counterpart_id)
  )
...
debt_breakdown |>
  ...
  geom_col() +
  labs(x = "External debt (in billion USD)", y = NULL,
       title = "Top 5 creditors of Zambia in 2022",
       subtitle = "External debt stocks, public and publicly guaranteed")
```

For more applications and insights into international debt, see [Teal Insights' Guide to Working with the World Bank International Debt Statistics](https://teal-insights.github.io/teal-insights-guide-to-wbids/).
